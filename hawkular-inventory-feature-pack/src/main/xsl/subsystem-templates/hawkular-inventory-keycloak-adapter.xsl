  ...
  <xsl:template match="//*[local-name()='config']/*[local-name()='subsystem']">
    <xsl:copy>
      ...
      <secure-deployment name="hawkular-inventory-dist.war">
        ...
        <credential name="secret"><xsl:value-of select="*[local-name()='secure-deployment' and @name='hawkular-accounts.war']/*[local-name()='credential' and @name='secret']/text()"/></credential>
      </secure-deployment>
    </xsl:copy>
  </xsl:template>
